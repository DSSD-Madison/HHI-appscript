...
export function isStakeholderApproved(e: GoogleAppsScript.Events.SheetsOnEdit): boolean {
...
export function isStakeholderRejected(e: GoogleAppsScript.Events.SheetsOnEdit): boolean {
  if (!isStakeholderStatusUpdate(e)) {
    return false
  }
...
export function isStakeholderStatusUpdate(e: GoogleAppsScript.Events.SheetsOnEdit): boolean {
  ...
  if (range.getNumRows() > 1 || range.getNumColumns() > 1) {
    if (DEBUG)
      Logger.log(`multiple columns affected, ignoring event`)
    return false
  }
  ...
  if (range.getColumn() !== APPROVAL_COLUMN_NUMBER) {
    if (DEBUG)
      Logger.log(`column mismatch ${range.getColumn()}, ${APPROVAL_COLUMN_NUMBER}`)
    return false
  }
